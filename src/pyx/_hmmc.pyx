...
def forward_backward_old(np.ndarray[DTYPE_t, ndim=2] state_trans_mat, np.ndarray[DTYPE_t, ndim=2] emission_seq, model_end_state=False):
	"""
	...
	cdef np.ndarray[DTYPE_t, ndim=1] s_j = np.ones((emission_seq.shape[0]), dtype=DTYPE)
	...
	forward[0, :] = state_trans_mat[0, 1:] * emission_seq[0, :]
	s_j[0] = sum(forward[0, :])
	forward[0, :] /= s_j[0]
	prev_forward = forward[0, :]
	#recursion step
	
	cdef np.ndarray[DTYPE_t, ndim=2] real_transitions = state_trans_mat[1:, 1:]
	...
	for sym_emission in emission_iterator:
		...
		np.dot(real_transitions.T, prev_forward, p_state_transition)
	...
	if model_end_state:
		prev_back = backward[emission_seq.shape[0] - 1, :] = (state_trans_mat[1:, 0])	 # Durbin p.60
	else:
		prev_back = backward[emission_seq.shape[0] - 1, :] = [1, 1]  # Rabiner p.7 (24)
	...
	for sym_emission in emission_seq[:0:-1]:
		np.dot(prev_back * sym_emission, real_transitions.T, p_state_transition)
